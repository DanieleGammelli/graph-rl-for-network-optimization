...
parser.add_argument('--s_store', type=int, default=16, metavar='S',
                    help='optimal store order-up-to-level (default: 16)')
parser.add_argument('--s_factory', type=int, default=19, metavar='S',
...
if not args.test:
    ...
    for i_episode in epochs:
        ...
        for step in range(T):
            ...
            # solve LCP
            action = solveLCP(env, ship, prod, CPLEXPATH=args.cplexpath, res_path='scim_1f2s')
    ...
    if args.algo == 'rl':
        ...
        model.load_checkpoint(path=f"./{args.directory}/pretrained/1f2s/graph_rl.pth")
