...
class GNNParser():
    ...
    """
    def __init__(self, env, T=10, grid_h=4, grid_w=4, scale_factor=0.01, json_file=None):
        super().__init__()
        self.env = env
        self.T = T
        self.s = scale_factor
    ...
    def parse_obs(self, obs):
        ...
        x = torch.cat((
            ...
            torch.tensor([[sum([(self.env.scenario.demand_input[i,j][t])*(self.env.price[i,j][t])*self.s \
                          if (i,j) in self.env.scenario.G.edges else 0 for j in self.env.region]) for i in self.env.region] for t in range(self.env.time+1, self.env.time+self.T+1)]).view(1, self.T, self.env.nregion).float()),
...
class A2C(nn.Module):
    ...
    """
    def __init__(self, env, input_size, json_file="data/scenario_nyc_brooklyn.json", 
    eps=np.finfo(np.float32).eps.item(), device=torch.device("cpu")):
