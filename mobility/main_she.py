...
parser.add_argument('--demand_ratio', type=int, default=2.5, metavar='S',
                    help='demand_ratio (default: 0.5)')
parser.add_argument('--json_hr', type=int, default=8, metavar='S',
...
scenario = Scenario(json_file="data/scenario_shenzhen_downtown_west.json", sd=args.seed, demand_ratio=args.demand_ratio, json_hr=args.json_hr, json_tstep=args.json_tsetp, tf=20)
...
if not args.test:
    ...
    for i_episode in epochs:
        ...
        for step in range(T):
            # take matching step
            obs, paxreward, done, info = env.pax_step(CPLEXPATH=args.cplexpath, PATH='scenario_she')
    ...
    for episode in epochs:
        ...
        while(not done):
            if args.algo == 'oracle':
                ...
                obs, paxreward, done, info = env.pax_step(CPLEXPATH=args.cplexpath, PATH='scenario_she_test')
                episode_reward += paxreward
                if args.algo == 'rl':
                ...
                rebAction = solveLCP(env,'scenario_she_test',desiredAcc,args.cplexpath)
                # Take action in environment
                new_obs, rebreward, done, info = env.reb_step(rebAction)
                episode_reward += rebreward
            # track performance over episode
